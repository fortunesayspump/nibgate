...
- Site routes: `POST /api/hub/sites/register`, `POST /api/hub/sites/:websiteId/verify`, `DELETE /api/hub/sites/:websiteId`.
- `POST /api/hub/pay` takes `{ price, recipient, title }` only.
