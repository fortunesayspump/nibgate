...
<body>
  <header class="topbar">
    <a class="brand brand-wordmark" href="/" aria-label="Nibgate home">NIBGATE</a>
    <nav>
      <a href="/">Home</a>
      <a href="${escapeHtml(exploreOrigin)}">Explore</a>
      <a class="${active === 'demo' ? 'active' : ''}" href="/demo/ghost/the-agent-economy">Example Content</a>
...
function unlockAction(route, gateway) {
  ...
  return `
    <div class="paywall">
      <div>
        <h2>Continue reading</h2>
        <p>This route is running in local example mode. Switch to Circle Gateway mode to use a live x402 payment instead of local simulation.</p>
      </div>
...
function creatorTools(route, gateway) {
  ...
  return `
    <div class="creator-tools">
      ...
      <p>This path uses the local server-side buyer wallet for fast local testing. It is not the normal reader checkout.</p>
      <form method="post" action="/api/content/${escapeHtml(route.id)}/unlock">
      ...
      <section class="article-layout">
        ...
        <aside class="side-panel">
          ...
          <dl>
            ...
            <dd>${gateway.paymentProvider.isLive ? 'Browser wallet + Gateway balance' : 'Local example mode'}</dd>
          </dl>
