...
//
// Run:  node scripts/seed-fixtures-e2e.mjs   (from backend/)
...
const RE = path.resolve(__dirname, '../../');

// ---- load backend/.env + subblogs .env ----
...
async function main() {
  ...
  // H9 expired (create active then backdate expiresAt in DB)
  const h9 = await createShare({ title: 'H9 expired fixture', slug: 'h9expired' });
  await db.nibShare.update({ where: { id: h9.id }, data: { expiresAt: new Date('2020-01-01T00:00:00Z') } });
  out.hub['expired'] = await db.nibShare.findUnique({ where: { id: h9.id } });
  ...
  const { PrismaClient } = await import(`${RE}/subblogs/backend/node_modules/@prisma/client/index.js`);
  const bp = new PrismaClient({ datasources: { db: { url: 'postgresql://fortune@localhost:5432/nibgate_blog' } } });

  const SUBSITE = 'stresslab';
  let site = await bp.site.findFirst({ where: { subdomain: SUBSITE } });
  if (!site) {
    site = await bp.site.create({
      data: { subdomain: SUBSITE, name: 'Stress Lab', description: 'e2e access-control fixture site', settings: null, verifyToken: null, createdAt: new Date(), updatedAt: new Date() },
    });
  }
  console.log('subblogs site:', site.id, site.subdomain);
  const out2 = out.subblogs;

  // Promote W_IMP user to admin on this site (create if absent) so its SIWE JWT
  // authorizes admin endpoints. Also create an author@ co-owner for authz tests.
  const upWallet = W_IMP.toLowerCase();
  const upEmail = `wallet-${upWallet.replace(/^0x/, '')}@wallets.nibgate.xyz`;
  let adminUser = await bp.user.findFirst({ where: { email: upEmail } });
  if (!adminUser) {
    adminUser = await bp.user.create({
      data: { siteId: site.id, name: 'E2E Admin', email: `wallet-${upWallet.replace(/^0x/, '')}@wallets.nibgate.xyz`,
        password: crypto.randomBytes(24).toString('hex'), role: 'admin', walletAddress: upWallet, createdAt: new Date(), updatedAt: new Date() },
    });
  } else {
    adminUser = await bp.user.update({ where: { id: adminUser.id }, data: { role: 'admin' } });
  }
  let otherUser = await bp.user.findFirst({ where: { siteId: site.id, email: 'e2e-other@nibgate.xyz' } });
  if (!otherUser) {
    otherUser = await bp.user.create({
      data: { siteId: site.id, name: 'Other Author', email: 'e2e-other@nibgate.xyz', password: crypto.randomBytes(24).toString('hex'), role: 'author', createdAt: new Date(), updatedAt: new Date() },
    });
  }
  console.log('subblogs adminUserId:', adminUser.id, ' otherAuthorId:', otherUser.id);

  // Wipe previous fixture posts on this site
  await bp.blogPostEntitlement.deleteMany({ where: { post: { siteId: site.id } } });
  await bp.blogPostReceipt.deleteMany({ where: { post: { siteId: site.id } } });
  ...
  // Entitlement/bans on subblogs via access.service (its prisma singleton reads DATABASE_URL).
// Both clients share globalThis.prisma; drop the hub client first so subblogs builds its own.
  delete globalThis.prisma;
  process.env.DATABASE_URL = 'postgresql://fortune@localhost:5432/nibgate_blog';
  const { default: access } = await import(`${RE}/subblogs/backend/src/services/access.service.js`);
