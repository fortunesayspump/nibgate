...
General-purpose encrypted blob storage. Two providers: free (R2, **live**) and paid
(Arweave, **planned — the API rejects `storageProvider: 'arweave'` today**).
...
| `nibshare/service.js` | Imports crypto + storage directly from `@nibgate/sdk/server` |
...
### Arweave tier (storageProvider: "arweave") — planned, not accepted by the API yet

- Creator pays network fee (~$0.001-0.01/KB via Irys)
- No size limit
- Arweave, permanent, decentralized
- Requires: Arweave provider registered in `server.js`, fee-estimation endpoint,
  creator wallet signing, and the UI/route accepting `storageProvider: "arweave"`.
...
6. Update `nibshare/service.js` — import crypto + storage from SDK, server-mode-only unlock, 512KB size cap
...
### Phase 3: Subblog content encryption at rest (done)

1. `BlogPost` gains `contentKey`, `bodyStorageRef`, `audioStorageRef`, `audioEncryptedKey`, `audioContentType` (nullable)
2. All subblog content is encrypted at rest with the SDK K-model (free AND paid):
   - Body: `generateContentKey` → `encryptBytes` → `packCipherBlob` → `putBlob` on create/update (`blog.service.js`); plaintext never written to `bodyMarkdown`
   - Media: `upload.route.js` supports `?encrypted=1`, returning `{ storageRef, encryptedKey, contentType }` instead of a public URL
   - Cover art stays on the plaintext free tier (public by design)
   - `GET /nibgate/access` decrypts the body server-side; free posts return it without proof, paid posts after `verifyUnlockToken`
   - `GET /nibgate/media/:postId/:kind` streams decrypted audio/photos (402 challenge on paid posts otherwise)
   ...
   - One-time backfill converts legacy plaintext rows: `subblogs/backend/scripts/backfill-encrypt-all.js`
3. Subblog UI: "Nibgate hosted" or "Arweave" selector (future)
