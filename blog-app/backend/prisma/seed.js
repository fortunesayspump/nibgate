...
async function main() {
  ...
  for (const a of articles) {
    await prisma.blogPost.create({ data: { ...a, tags: 'article,' + a.tags, siteId: site.id, authorId: author.id } });
  }
  for (const p of photoGalleries) {
    await prisma.blogPost.create({ data: { ...p, tags: 'photo,' + p.tags, bodyMarkdown: '', type: 'photo', media: JSON.stringify(p.media), siteId: site.id, authorId: author.id } });
  }
  for (const m of musicPosts) {
    await prisma.blogPost.create({ data: { ...m, bodyMarkdown: m.excerpt, tags: 'music,' + m.excerpt.slice(0, 30), type: 'music', siteId: site.id, authorId: author.id } });
  }
  for (const v of videoPosts) {
    await prisma.blogPost.create({ data: { ...v, bodyMarkdown: v.excerpt, tags: 'video,' + v.excerpt.slice(0, 30), type: 'video', siteId: site.id, authorId: author.id } });
  }
