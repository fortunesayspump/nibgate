    ...
    async function init() {
      try {
        ...
        createEvmGatewayUnlock(resource, {
          ...
          circleClientModuleUrl: "https://esm.sh/@circle-fin/x402-batching@3.2.0/client?bundle",
          onUnlock() {
    ...
    <aside className="border border-[var(--border)] bg-[var(--surface)] rounded-lg p-6 my-8">
      ...
      <div className="border border-[var(--border)] rounded-lg p-4 mb-6 space-y-3">
        ...
        <div className="flex gap-3">
          <button
            type="button"
            data-nibgate-connect
            className="border border-[var(--border)] rounded-md px-4 py-2 text-xs font-semibold hover:bg-[var(--card-hover)] transition-colors cursor-pointer"
          >
            Connect
          </button>
          <button
            type="button"
            data-nibgate-disconnect
            disabled
            className="border border-[var(--border)] rounded-md px-4 py-2 text-xs text-[var(--muted)] cursor-pointer disabled:opacity-40"
          >
            Disconnect
          </button>
        </div>
      ...
      <button
        type="button"
        data-nibgate-unlock
        className="w-full bg-[var(--fg)] text-[var(--bg)] border-0 rounded-md px-6 py-3 text-sm font-semibold hover:opacity-90 transition-opacity cursor-pointer"
      >
        Unlock for {resource.price} USDC
      </button>

      <p className="text-xs text-center text-[var(--muted)] mt-4" data-nibgate-status>
        {status || (typeof window !== "undefined" && window.ethereum
      ...
      <div data-nibgate-premium hidden className="mt-6 border-t border-[var(--border)] pt-6">
        <span className="text-[10px] font-semibold uppercase tracking-widest text-[var(--accent)] border border-[var(--accent)] rounded px-2 py-0.5">
          Unlocked
        </span>
