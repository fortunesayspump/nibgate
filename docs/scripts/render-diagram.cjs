...
const TITLE = process.env.DIAGRAM_TITLE || (() => { console.warn('WARN: DIAGRAM_TITLE not set, using default'); return 'Nibgate Flow at a Glance'; })();
...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    window.mermaid.initialize({ startOnLoad: false, theme: 'default', fontFamily: 'Kumbh Sans, ABC Favorit, Arial, sans-serif', fontSize: ${FONT_SIZE}, sequence: { actorMargin: ${ACTOR_MARGIN} }, flowchart: { htmlLabels: false } });
...
function recolorForDark(svg) {
  ...
  return svg
    ...
    .replace('.cluster rect{fill:#ffffde;stroke:#aaaa33;stroke-width:1px;}', `.cluster rect{fill:${NOTE};stroke:${BORDER};stroke-width:1px;}`)
    .replace('.cluster-label span{color:#333;}', `.cluster-label span{color:${LIGHT};}`)
    .replace('.cluster text{fill:#333;}', `.cluster text{fill:${LIGHT};}`)
    .replace('.cluster span{color:#333;}', `.cluster span{color:${LIGHT};}`)
    .replace('.labelBkg{background-color:rgba(232, 232, 232, 0.5);}', '.labelBkg{background-color:rgba(23,24,19,0.85);}')
    .replace('.cluster-label .node text{}', '');
}

function padNoteRects(svg, padX = 14, padY = 10) {
  return svg.replace(
    /<rect x="(-?[\d.]+)" y="(-?[\d.]+)" ((?:[^>](?!class="note"))*)width="([\d.-]+)" height="([\d.-]+)" class="note"/g,
    (m, x, y, mid, w, h) => `<rect x="${+x - padX}" y="${+y - padY}" ${mid}width="${+w + padX * 2}" height="${+h + padY * 2}" class="note"`
  );
}
...
(async () => {
  ...
  const padded = padNoteRects(final);

  fs.writeFileSync(out.replace(/\.png$/, '.svg'), padded);
