...
sequenceDiagram
    ...
    participant S as Creator (subblog)
    participant W as Wallet (viem)
    participant C as Circle Gateway

    A->>H: 1. GET /.well-known/llms.txt (or MCP explore_content)
    H-->>A: Curated index + endpoint map
    A->>H: 2. GET /api/hub/explore/content?sort=trending
    H-->>A: Content cards (price, creator, signals)
    A->>S: 3. GET /manifest → evaluate paid vs free
    S-->>A: Structured metadata, access policy
    A->>S: 4. GET /api/nibgate/access?slug=... (x-nibgate-actor: agent)
    S-->>A: 402 + PAYMENT-REQUIRED (base64 x402 v2 challenge)
    A->>W: 5. Sign EIP-3009 TransferWithAuthorization (gasless)
    W-->>A: v,r,s + nonce
    A->>S: 6. Retry with Payment-Signature header
    S->>C: 7. Verify + settle via Gateway
    C-->>S: txHash
    S-->>A: 200 + content + unlockProof + payment
    A->>H: 8. POST /api/hub/reputation/ratings/prepare
    H-->>A: Signable message + contentHash + contract
    A->>W: 9. Sign rating (rateContent calldata)
    A->>H: 10. POST /api/hub/reputation/ratings/index { txHash }
...
```bash
curl -i 'https://thedailybyte.nibgate.xyz/api/nibgate/access?slug=nolan-director' \
...
const result = await agent.pay(
  'https://thedailybyte.nibgate.xyz/api/nibgate/access?slug=nolan-director',
...
5. Return the unlocked response
