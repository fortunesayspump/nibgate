...
The default allows all well-behaved crawlers and points to the sitemap:
...
The main Nibgate hub at `nibgate.xyz` serves its own `/sitemap.xml` for hub pages. Subblogs are NOT aggregated into a hub-level sitemap — Google only follows sitemap references on the same host, so cross-host aggregation doesn't work. Instead, every Subblog handles its own discovery via its own `robots.txt` + `sitemap.xml`, exactly like Substack:
...
The `@type` varies by content type:
...
<meta property="og:url" content="https://yourname.nibgate.xyz/posts/slug" />
