...
The main Nibgate hub at `nibgate.xyz` serves `/subblog-sitemaps.xml`, which aggregates sitemaps from all Subblogs. This gives search engines a single entry point to discover every Subblog:

```txt
https://nibgate.xyz/subblog-sitemaps.xml
```

This aggregate sitemap is automatically updated as new Subblogs are created and as content is published or updated.
