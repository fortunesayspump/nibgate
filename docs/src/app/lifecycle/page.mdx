...
The package reports content metadata to Nibgate: content id, type, title, route, price, tags, image, verified site, and creator wallet.

The widget on the creator site streams activity to `/hub/evt`: page views, resource views, time spent, scroll depth, and unlock/payment events. Each resource event carries the resource id, title, type, price, path, and image URL.
...
On Nibgate-hosted surfaces (the hub widget checkout and Subblogs), the receiver resolves to the creator's `GatewayFeeWallet` contract — an immutable per-creator contract that keeps 99% for the creator and routes a 1% protocol fee to the treasury. See [Revenue model](/revenue-model).
...
**Server-side x402 unlock** — `accessResponse()` and `payAndUnlockResponse()` call `emitHubEvent()` after successful gateway payment, sending `payment_completed` and `unlock_completed` events to the Hub automatically.

```ts
guide.unlockCompleted({
  payer: "0x...",            // buyer wallet (also accepts walletAddress | buyer | account | address)
  paymentProvider: "x402",
  txHash: "0x...",
  receiptUrl: "https://...", // gateway/provider receipt reference
  amount: "0.01",
  currency: "USDC"
})
```
