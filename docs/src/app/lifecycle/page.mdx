...
<Callout type="info">
For multipublisher platforms, the platform verifies the domain and the creator verifies a publisher identity with a wallet. The same wallet should be used on the platform and on Nibgate so the dashboard can show the creator's route, content, metrics, earnings, and reputation.
</Callout>
...
platform wallet
  ...
  -> creator wallet claims @handle
  -> package-installed platform content
  -> publisher-scoped events and receipts
  -> creator dashboard publisher identity
```
...
If the creator publishes on a platform, this should be the same wallet they connected on the platform. That match lets Nibgate link `platform.com/@handle` to the creator's Nibgate dashboard.
...
A creator should only register a domain they control. On a multipublisher platform, the platform owner registers the domain. The creator claims a publisher identity instead of adding the platform as a site.
...
On a platform, the resource should also include `publisher.id`, `publisher.handle`, `publisher.walletAddress`, and `publisher.profileUrl`. That is how events from one verified domain become Alice's metrics instead of only platform-level metrics.
...
For platform content, Nibgate should also index the publisher id, publisher wallet, profile route, and payment recipient used for that resource.
...
For platform publishers, the dashboard should show a Publisher Identities section, such as `@alice on platform.com`, and roll content, metrics, earnings, and reputation into the wallet-linked creator account.
