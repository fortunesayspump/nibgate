...
The eligible wallet can submit a rating proof or onchain rating event, depending on the site/product configuration. The package should resolve the content id, check unlock eligibility, and hand off to the configured rating submission path once that adapter exists.
...
```txt
Creator installs package -> verifies site -> publishes metadata -> users/agents discover on Nibgate -> unlock on creator site -> package reports receipt -> eligible wallets rate -> Nibgate indexes reputation -> Explore, leaderboards, and dashboard update.
```
