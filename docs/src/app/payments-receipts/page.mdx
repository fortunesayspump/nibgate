...
- `GET /api/nibshare/gateway/balance` (hub) and `GET /api/gateway/balances` (subblogs) — return the depositor's testnet USDC balance (needs `CIRCLE_API_KEY` set on the backend; response shows `{ balance: "0.00 USDC" }`).
