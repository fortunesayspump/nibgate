...
Nibgate does not need to custody funds to show earnings. It records payment events and receipt references after the creator site completes an unlock. Receipts store the gross `amount` plus a derived `protocolFee` (1% under hosted pay), so analytics match the fee wallet contract's on-chain 99/1 split — see [Revenue model](/revenue-model).
</Callout>
