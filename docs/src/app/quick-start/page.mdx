...
The `unlock` policy field already supports metered streaming, metered reading, time passes, and agent quotas in the resource schema; their payment/session adapters are not production-ready yet, so `one_time` is the shipped path.
