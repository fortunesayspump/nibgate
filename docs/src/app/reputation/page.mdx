...
Nibgate records the rating with its on-chain proof and updates content stars. Read them back via `GET /hub/reputation/ratings/stats?contentId=` — the hub-authoritative source (average 1–5, count).
...
A site has a verification state — `verified`, `missing_widget` (widget marker no longer on the homepage), `failed` (unreachable or invalid), or soft-deleted (`deletedAt`) — plus content-level freshness and quality signals.
