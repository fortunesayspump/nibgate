...
> Deep dive: how addresses exist before deployment, why wallets are keyed per content recipient, keeper discovery, and ghost recovery → [Fee wallets & the keeper](/revenue-model/fee-wallets).
...
### Discovery
The keeper lists payout addresses from four redundant sources — nibshare owners, website owners with linked wallets, per-post `recipientWallet` on priced content, and every address that ever received a verified payment — then predicts each one's fee wallet address. Any address that can earn (or ever earned) is covered.
