...
It fails **closed**: if the registry rejects the hash for different content → `402 txhash-claimed-elsewhere`; if the registry is unreachable → `503 claim-registry-unavailable`, never a silent unlock. Off by default because single-site integrators don't need the dependency.
