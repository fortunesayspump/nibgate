...
- **`@nibgate/wallet` (0.4.x)** published on npm: shared Reown AppKit (adapter-less — no wagmi) wallet provider, SIWE sign-in, and the `<NibgateUnlock>`/`GatewayWallet` single-checkout flow used by the hub header and both rails (`./react` entry). Access follows the wallet: paid content re-verifies from the wallet's receipt on reconnect (no re-pay) and relocks on disconnect.
