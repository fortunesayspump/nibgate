...
(async () => {
  ...
  console.log('after import click:', txt);
  // look for error
  const err = home.locator('[class*="error" i], [class*="danger" i]').first();
  if (await err.count()) console.log('ERR EL:', (await err.innerText()).slice(0,120));
  await context.close();
  process.exit(0);
