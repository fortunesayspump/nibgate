...
async function main() {
  ...
  if (await cont.count()) {
    console.log('continue exists; visible:', await cont.isVisible());
    const box = await cont.boundingBox();
    console.log('box:', JSON.stringify(box));
    // try click at center via mouse with coords
    if (box) {
      await home.mouse.click(box.x + box.width / 2, box.y + box.height / 2);
      await home.waitForTimeout(1500);
    }
    console.log('after click center, body:', (await home.locator('body').innerText()).replace(/\s+/g,' ').slice(0,120));
  } else {
    console.log('no continue button; body:', (await home.locator('body').innerText()).replace(/\s+/g,' ').slice(0,160));
  }
  await context.close();
  process.exit(0);
}
