...
async function main() {
  ...
  // home.html: try clicking "Open wallet"
  ...
  const done = home.locator('[data-testid="onboarding-complete-done"]').first();
  if (await done.count()) {
    console.log('found open-wallet, clicking');
    await done.click({ force: true });
    await home.waitForTimeout(2500);
  }
  // now dump all buttons and check for account menu / network
  ...
  // check url / hash
  console.log('current url:', home.url());
