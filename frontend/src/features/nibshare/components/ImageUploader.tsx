...
export default function ImageUploader({
  ...
  async function addFiles(fileList: FileList | File[]) {
    ...
    for (const file of fileArray) {
      ...
      try {
        ...
        fd.append("file", file);
        const data = await uploadJson(`${UPLOAD_URL}${encrypted ? "?encrypted=1" : ""}`, fd);

        const item: ContentMedia = encrypted
          ? { storageRef: data.storageRef, encryptedKey: data.encryptedKey, contentType: data.contentType, previewUrl: data.previewUrl, caption: "" }
  ...
  function handleCoverSelect(index: number) {
    ...
    onCoverChange(item.previewUrl || item.url || "", key);
  }
            ...
            <div key={i} style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              <div style={{ position: "relative", borderRadius: "6px", overflow: "hidden", border: `1px solid ${isCover ? "var(--accent)" : "var(--border)"}`, aspectRatio: "1" }}>
                {item.url || item.previewUrl ? (
                  <img src={item.previewUrl || item.url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                ) : (
