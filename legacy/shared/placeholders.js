...
export function arrowIconDataUri(direction = 'right') {
  const rotation = direction === 'left' ? 'rotate(180 16 16)' : '';
  return svgDataUri(`
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
      <rect width="32" height="32" rx="16" fill="${palette.paper}" stroke="${palette.ink}" />
      <g transform="${rotation}">
        <path d="M11 16h10" stroke="${palette.ink}" stroke-width="2.5" stroke-linecap="round"/>
        <path d="m17 10 6 6-6 6" fill="none" stroke="${palette.ink}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
      </g>
    </svg>
  `);
}

export function placeholderAvatar(label, { size = 128 } = {}) {
...
export function placeholderArt(title, { kicker = 'Nibgate', width = 800, height = 800 } = {}) {
...
export function placeholderTile(label, { size = 240 } = {}) {
  const colors = tone(label);
...
export function siteImagePlaceholder(path) {
  ...
  if (path.includes('/creators/')) {
    return placeholderAvatar(label, { size: 128 });
  }

  if (path.includes('/discover/')) {
    return placeholderTile(label, { size: 160 });
  }

  return placeholderArt(label, {
    kicker: path.includes('/about/') ? 'Nibgate' : 'Explore',
    width: 960,
    height: 720
  });
}
