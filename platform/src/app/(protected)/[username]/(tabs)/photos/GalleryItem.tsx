...
export function GalleryItem({ type, url, onClick }: { type: VisualMediaType; url: string; onClick: () => void }) {
  ...
  return (
    <div
      ...
      className={cn('group relative focus:outline-none', isFocusVisible && 'border-4 border-emerald-700')}>
      {type === 'PHOTO' ? (
        ...
        <>
          <Play
            width={36}
            height={36}
            className="absolute left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] stroke-emerald-50 transition-transform group-hover:scale-125"
          />
