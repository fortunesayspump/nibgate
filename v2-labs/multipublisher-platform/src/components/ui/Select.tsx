      ...
      <>
        <div className="relative">
          ...
          <button
            ...
            type="button"
            className={cn(
              'absolute left-5 cursor-pointer font-semibold text-slate-500 transition-all',
              isThereASelectedValue ? 'top-[9px] translate-y-0 text-xs' : 'top-[50%] translate-y-[-50%] text-sm',
              Icon ? 'left-16' : 'left-5',
              isError ? 'text-destructive-foreground' : 'text-slate-500',
            )}
            onClick={open}>
            {label}
          </button>
          ...
          <button
            ...
            ref={ref}
            className={cn(
              'w-full rounded-md border border-border bg-white pb-2 pl-5 pr-5 pt-8 text-left text-sm text-slate-950 outline-none ring-emerald-700 focus:border-emerald-300 focus:ring-2',
              Icon ? 'pl-16' : 'pl-5',
              isError && 'border-red-200 bg-destructive ring-destructive-foreground focus:ring-4',
            )}>
            <span
              {...valueProps}
            ...
            {!isThereASelectedValue && (
              <div className="absolute right-5 top-[50%] z-[1] translate-y-[-50%] p-3">
                <SvgArrowChevronDown
                  className={cn('h-5 w-5 stroke-slate-500 transition-transform', state.isOpen && 'rotate-180')}
                />
              </div>
            )}
          </button>
          ...
          <div className="absolute right-5 top-[50%] z-[1] translate-y-[-50%]">
            {isThereASelectedValue && (
              <Button
                Icon={SvgClose}
                iconClassName="stroke-slate-500"
                mode="ghost"
                size="small"
                onPress={clear}
                aria-label="Clear"
